...
class BlueprintEditor {
    ...
    _bindImageDrop() {
        ...
        c.addEventListener('drop', e => {
            ...
            this._loadImageFile(file, blob => {
                const pos = this._toCanvas(e.clientX, e.clientY);
                this._createImageNode(blob, pos.x - 80, pos.y - 60);
            });
        });

        // 粘贴图片（支持 GIF / PNG / JPG 等所有格式）
        document.addEventListener('paste', e => {
            // 如果焦点在输入框内，不拦截粘贴
            const tag = document.activeElement?.tagName;
            if (tag === 'INPUT' || tag === 'TEXTAREA') return;
            const contentEditable = document.activeElement?.isContentEditable;
            if (contentEditable) return;

            const items = [...(e.clipboardData?.items || [])];
            // 优先选 GIF（Chrome 有时同时提供 image/gif 和 image/png 两个 item）
            const imageItem = items.find(i => i.type === 'image/gif')
            ...
            this._loadImageFile(blob, resolvedBlob => {
                const c2 = this._toCanvas(
                    this.container.clientWidth  / 2,
                    this.container.clientHeight / 2
                );
                this._createImageNode(resolvedBlob, c2.x - 80, c2.y - 60);
            });
        });
    }
    ...
    /**
     * 统一的图片文件加载方法
     * GIF 使用 base64（保留动画，且页面刷新后 src 不失效）
     * 其他格式也使用 base64（保证持久化一致性）
